...
function index() {
  ...
  return (
    <div className='bg-black w-full h-full pb-10'>
      <Navbar/>    
      <div className='flex flex-col px-10 pt-32 overflow-hidden text-white'>
      <div className='flex items-center absolute top-20 left-[50px] text-white'>
                  <span className='text-3xl font-medium'>Movies</span>
      ...
      {category?.map((genre) => (
        <div key={genre.name} className='group/link mt-5'>
      <Link href={`/c/${genre.id}`}>
          <div className='flex items-center w-fit rounded-xl gap-1 px-3 group/open hover:bg-zinc-800 cursor-pointer'>
            <h2 className='text-lg font-medium'>{genre.name}</h2>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 hidden group-hover/link:flex group-hover/open:translate-x-1 delay-100">
