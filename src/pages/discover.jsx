        ...
        <div className='w-full h-full pb-10 bg-black'>
            <div className='fixed w-screen h-screen bg-black -z-50'></div>
                ...
                <div className='flex justify-center pt-28'>
                    ...
                    <input className='fixed z-50 w-32 text-white bg-transparent top-7 right-52 outline-0 placeholder:text-gray-500' onChange={(e) => getMoviesBySearch(e?.target?.value)} type='text' placeholder='Search'/>
                </div>
                <div className='flex flex-wrap items-center justify-center gap-5'>
                    {(!searchedMovies ? movieData : searchedMovies)?.map((movie) => 
                        <div 
                            onMouseEnter={() => {setMovieId(movie.id); findMovies(movie.id);}} onMouseLeave={() => setMovieId()} 
                        ...
                        {movie.backdrop_path !== null && (
                            ...
                            { !favoriteStatus && (
                                <svg onClick={() => addMovie(movie.id, movie.title, movie.backdrop_path)} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 cursor-pointer">
